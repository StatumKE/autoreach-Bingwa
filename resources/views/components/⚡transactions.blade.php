        ...
        <div class="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-green-50 dark:bg-green-950/20 px-3 py-2.5 ring-1 ring-green-100 dark:ring-green-900/50">
            ...
            <div class="flex items-center gap-1.5 overflow-x-auto no-scrollbar">
                ...
                <flux:button
                    ...
                    wire:loading.attr="disabled"
                    class="app-secondary-button !h-7 px-2 shrink-0 text-[9px] font-bold uppercase tracking-wider"
                >
                    <span wire:loading.remove wire:target="retrySelectedTransactions">
                        {{ __('Retry') }}
                    </span>
                    <span wire:loading wire:target="retrySelectedTransactions" class="inline-flex items-center gap-1">
                        <flux:icon.loading variant="mini" class="size-3" />
                        {{ __('Wait…') }}
                    </span>
                </flux:button>
                ...
                >
                    <div class="flex items-center justify-between gap-2">
                        <div class="flex items-center gap-2">
                            <div class="relative flex items-center size-4 cursor-pointer">
                                <input
                                    type="checkbox"
                        ...
                        <div class="flex min-w-0 flex-col items-end gap-0.5 text-right">
                            ...
                            <span class="text-[9px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest whitespace-nowrap">
                                {{ \App\Support\AppTimezone::format($transaction->occurred_at, 'H:i, M j') }}
                            </span>
                        </div>
                    </div>
                    ...
                    @if ($isFailed && $transaction->next_attempt_at)
                        <div class="mt-2 rounded-lg bg-amber-50 px-2 py-1.5 text-[10px] font-medium leading-snug ring-1 ring-amber-100 dark:bg-amber-500/10 dark:text-amber-300 dark:ring-amber-500/20">
                            ...
                            {{ __('Marked for auto-renewal tomorrow at :time.', ['time' => \App\Support\AppTimezone::format($transaction->next_attempt_at, 'H:i, M j')]) }}
                        </div>
                    @endif
