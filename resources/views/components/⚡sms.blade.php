...
<section class="min-h-screen bg-app-bg px-4 pb-24 pt-3" wire:init="loadPage">
    ...
    <div class="flex flex-col gap-3">
        <div class="flex items-center justify-between gap-3 px-1">
            <div>
                <div class="text-xl font-bold text-zinc-900">{{ __('SMS History') }}</div>
        ...
        <div class="flex flex-col gap-3">
            <flux:input
                ...
                class="rounded-xl bg-white shadow-sm ring-1 ring-zinc-200 text-zinc-900"
            >
                <x-slot name="icon">
            ...
            <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                @foreach ([
                    ...
                    'sending' => ['label' => __('Sending'), 'active' => 'bg-sky-500 text-white shadow-sm ring-1 ring-inset ring-sky-600/20', 'inactive' => 'bg-white text-zinc-500 ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50 hover:text-zinc-900'],
                    'sent' => ['label' => __('Sent'), 'active' => 'bg-green-600 text-white shadow-sm ring-1 ring-inset ring-green-700/20', 'inactive' => 'bg-white text-zinc-500 ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50 hover:text-zinc-900'],
                    ...
                    'skipped' => ['label' => __('Skipped'), 'active' => 'bg-zinc-700 text-white shadow-sm ring-1 ring-inset ring-zinc-800/20', 'inactive' => 'bg-white text-zinc-500 ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50 hover:text-zinc-900'],
                ] as $value => $styles)
                    <flux:button
                        type="button"
                        variant="ghost"
                    ...
                    <article class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-200">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <div class="mt-1 text-[11px] font-medium text-zinc-500">
                        ...
                        <div class="mt-4 grid gap-3 text-sm text-zinc-700">
                            <div class="rounded-xl bg-zinc-50 p-3">
                                <div class="text-[10px] font-black uppercase tracking-widest text-zinc-500">{{ __('Message') }}</div>
                                <div class="mt-1 leading-relaxed text-zinc-900">
                                    {{ $this->messagePreview($message->auto_reply_message) }}
                                </div>
                            </div>

                            <div class="grid gap-2 text-[11px] font-medium text-zinc-500 sm:grid-cols-2">
                                <div class="rounded-xl bg-zinc-50 p-3">
                                    <div class="text-[10px] font-black uppercase tracking-widest text-zinc-400">{{ __('Buyer Phone') }}</div>
                                    <div class="mt-1 text-zinc-900">{{ $this->buyerPhone($message) }}</div>
                                </div>

                                <div class="rounded-xl bg-zinc-50 p-3">
                                    <div class="text-[10px] font-black uppercase tracking-widest text-zinc-400">{{ __('SIM Used') }}</div>
                                    <div class="mt-1 text-zinc-900">{{ $this->simLabel($message->auto_reply_sim_slot) }}</div>
                                </div>

                                <div class="rounded-xl bg-zinc-50 p-3">
                                    <div class="text-[10px] font-black uppercase tracking-widest text-zinc-400">{{ __('Attempts') }}</div>
                                    <div class="mt-1 text-zinc-900">{{ number_format((int) $message->auto_reply_attempts) }}</div>
                                </div>
                            ...
                            @if (filled($message->auto_reply_failure_reason))
                                <div class="rounded-xl bg-rose-50 p-3 text-rose-700 ring-1 ring-rose-100">
